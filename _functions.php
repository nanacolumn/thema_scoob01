...
function mkCalendar(){
	...
	$last_cnt = $last+1;
	$first_week = date('N', mktime(0, 0, 0,$_SESSION['month'],1,$_SESSION['year']));
	$last_week = date('N', mktime(0, 0, 0,$_SESSION['month'],$last,$_SESSION['year']));

	//祝日
	if( !isset($_SESSION['holidays']) || $_SESSION['holidays']['key'] != date('Ym') ){
		unset($_SESSION['holidays']);
		$_SESSION['holidays']['key'] = date('Ym');
	...
	$data['today'] = ($_POST['date'] != "")? $_POST['date']: date('Ymd', strtotime('+9 hour'));
	if( $_SESSION['year'].sprintf('%02d', $_SESSION['month']) > date('Ym') ){
		//prev表示
		$data['p'] = 1;
	}
	if( $_SESSION['year'].sprintf('%02d', $_SESSION['month']) < (date('Ym', strtotime('+2 month'))) ){
		//next表示
		$data['n'] = 1;
	}
...
function getHolidays(){
	$ym01 = date('Y-m-01');
	$ymt = date('Y-m-t', strtotime($ym01 . '+2 month'));
	...
	if ( $results = file_get_contents($holidays_url) ) {
        ...
        foreach ($results['feed']['entry'] as $val ) {
                ...
                $title = $val['title']['$t'];
                $holidays[date('Ymd', strtotime($date))] = 'holiday';
        }
	}
